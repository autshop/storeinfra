...
JSON_STRING=$( jq -n \
                  ...
                  '[{name: "TENANT_ID", value: $tid}, {name: "REACT_APP_SERVER_URL",value: $s }]' )

echo $JSON_STRING

CODEBUILD_RESPONSE=$(aws codebuild start-build --project-name shop-storeadmin --environment-variables-override "$JSON_STRING")
